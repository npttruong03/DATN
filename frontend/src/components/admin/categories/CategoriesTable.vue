<template>
    <div class="bg-white rounded-lg shadow p-4">
        <div class="flex justify-end mb-4" v-if="props.categories && props.categories.length > 0">
            <button v-if="selectedCategories.size > 0" @click="$emit('bulkDelete', selectedCategories)"
                ...
                <span class="hidden sm:inline">Xóa {{ selectedCategories.size }} mục đã chọn</span>
                <span class="sm:hidden">Xóa {{ selectedCategories.size }}</span>
            </button>
        </div>
        ...
        <div class="table-container">
                ...
                <div class="overflow-x-auto rounded-2xl border border-gray-200 bg-white">
                    <table class="w-full text-sm min-w-[800px]">
                                ...
                                <th class="px-3 py-2 text-center">
                                    <div class="flex items-center justify-center">
                                        <input type="checkbox"
                                            ...
                                            :indeterminate="selectedCategories.size > 0 && selectedCategories.size < paginatedCategories.length"
                        ...
                        <tbody>
                            ...
                            <tr v-else-if="props.categories && props.categories.length > 0"
                                ...
                                <td class="px-3 py-2 text-center">{{ (props.currentPage - 1) * props.itemsPerPage +
                                    index + 1 }}
                                </td>
                                ...
                                <td class="px-3 py-2 text-center max-w-xs truncate" :title="category.description">{{
                                    category.description }}</td>
                                <td class="px-3 py-2 text-center">{{ getParentCategoryName(category.parent_id) }}</td>
                            ...
                            <tr v-if="!props.isLoading && props.categories && props.categories.length === 0">
            ...
            <div class="mobile-cards">
                ...
                <div v-else-if="props.categories && props.categories.length > 0"
                    v-for="(category, index) in paginatedCategories" :key="'mobile-' + category.id" class="mobile-card">
                    <div class="card-header">
                        <div class="card-checkbox">
                            <input type="checkbox" :checked="selectedCategories.has(category.id)"
                                @change="toggleSelect(category.id)" />
                        </div>
                        <div class="card-number">{{ (props.currentPage - 1) * props.itemsPerPage + index + 1 }}</div>
                    </div>
                    ...
                    <div class="card-actions">
                        <router-link :to="`/admin/categories/edit/${category.id}`" class="mobile-action-btn edit-btn">
                            <i class="fas fa-edit"></i>
                            <span>Sửa</span>
                        </router-link>
                        <button @click="handleDelete(category)" class="mobile-action-btn delete-btn">
                            <i class="fas fa-trash"></i>
                            <span>Xóa</span>
                        </button>
                    </div>
                ...
                <div v-if="!props.isLoading && props.categories && props.categories.length === 0" class="empty-state">
        ...
        <!-- Pagination -->
        <div v-if="!props.isLoading && props.categories && props.categories.length > 0 && totalPages > 1"
            class="flex flex-col sm:flex-row justify-between items-center mt-6 gap-4">
            <div class="text-sm text-gray-600 text-center sm:text-left">
                Hiển thị {{ paginatedCategories.length }} trên tổng số {{ props.categories.length }} bản ghi
            </div>
            <div class="flex gap-2">
                <button :disabled="props.currentPage === 1" @click="goToPage(props.currentPage - 1)"
                ...
                <button :disabled="props.currentPage === totalPages" @click="goToPage(props.currentPage + 1)"
                    class="px-3 py-1 border border-gray-400 rounded hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors cursor-pointer">
...
// Thêm function để lấy tên parent category an toàn
...
const toggleSelectAll = (event) => {
    if (event.target.checked) {
        selectedCategories.value = new Set(paginatedCategories.value.map(category => category.id))
    } else {
...
const paginatedCategories = computed(() => {
    ...
    const result = props.categories.slice(start, end)
    return result
})
