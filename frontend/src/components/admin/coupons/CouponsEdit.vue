...
const handleSubmit = async () => {
    ...
    try {
        await updateCoupon(route.params.id, formData.value)
        push.success('Cập nhật khuyến mãi thành công')
        router.push('/admin/coupons')
    } catch (err) {
        push.error('Lỗi cập nhật khuyến mãi')
        console.error(err)
    }
}
...
const fetchCoupon = async () => {
    try {
        const res = await getCouponById(route.params.id)
        formData.value = {
            ...res,
            is_active: !!res.is_active,
        }
    } catch (err) {
        push.error('Không thể tải dữ liệu mã giảm giá')
        console.error(err)
        router.push('/admin/coupons')
    }
}
